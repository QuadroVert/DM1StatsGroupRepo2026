...
# Paths are resolved relative to this file so imports work regardless of
# which directory the calling notebook is run from.
_data = Path(__file__).parent.parent / 'SourceData'

bins    = pd.read_csv(_data / 's2_binning_info.csv')
resp_nr = pd.read_csv(_data / 's2_response_nr.csv')
resp_er = pd.read_csv(_data / 's2_response_er.csv')
bg      = pd.read_csv(_data / 'er_and_cevns_background.csv')
events  = pd.read_csv(_data / 'events_after_cuts.csv')
